...
func main() {
	...
	detectAdd := members
	detectAdd = append(detectAdd, secretaries...)
	//detectAdd = append(detectAdd, observers...)
	sv := Server{
		address: 		address,
		members: 		members,
	...
	sv.rf = BW_RAFT.MakeBWRaft(sv.address, sv.members, sv.secretaries, sv.detectAdd, sv.persist, sv.mu) // 一直不会返回结果
